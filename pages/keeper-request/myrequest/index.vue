...
const field = ["Id", "Pet", "Pet Owner", "Date", "Status", "Action"]
...
<template>
  <div>
    ...
    <rs-card class="flex-wrap justify-between p-10 bg-white text-black rounded-md flex-col">
      <rs-table :field="field" :data="myrequest.data" :options="{
        variant: 'default',
        striped: true,
        bordered: true,
        borderless: true,
        ...
        <template v-slot:Date="data">
          <span>{{ moment(data.value.requestAdoptionDateAppointment ).add(25, 'minutes').format("DD-MM-YYYY HH:MM:ss") }}</span>
        </template>

        <template v-slot:Status="data">
          <rs-badge :variant="data.value.requestAdoptionStatus == 'Pending' ? 'warning' : 'success'">
            {{ data.value.requestAdoptionStatus }}
          </rs-badge>
